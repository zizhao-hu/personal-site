...
WebLLM enables running large language models directly in your browser using WebGPU acceleration. This means you can build AI-powered applications without sending data to external serversâ€”everything runs locally on the user's device.
...
class PretrainingDataset(Dataset):
    ...
    def __init__(
        ...
        print(f"Dataset: {len(self.data):,} tokens â†’ {self.n_chunks:,} chunks")
...
*Next: Stage 1 â€” Pretraining, where we build the model and write the training loop.*

---
# Stage 1: Pretraining

Pretraining is where a language model learns to predict the next token. This is the most compute-intensive phase â€” you're teaching the model the statistical patterns of language from raw text.
...
Perplexity is the standard metric for pretraining â€” lower is better. A perplexity of 20 means the model is "20-way confused" on average.
...
| Gradient norm | Stable around 0.1â€“1.0 | Exploding (>10) or vanishing |
| Learning rate | Smooth warmup â†’ cosine decay | â€” |
...
*Next: Stage 2 â€” Instruction Tuning, where we teach the model to follow instructions.*

---
# Stage 2: Instruction Tuning

A pretrained model can complete text, but it can't follow instructions. Instruction tuning (SFT â€” Supervised Fine-Tuning) teaches the model to respond helpfully to user requests. This is the step that turns a "text completer" into a "chatbot."
...
class InstructionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        y[:len(ids) // 2] = -1  # Simplified â€” mask system + user tokens
        return x, y
\`\`\`
...
*Next: Stage 3 â€” Reasoning, where we teach the model to think step by step.*
...
60 km/h = 60 Ã— 1000 / 3600 = 16.67 m/s.""",
...
Total discount is not 35% â€” it's 25% then 10% of the reduced price.""",
...
preference_pairs = [
    ...
    {
        "prompt": "Is water wet?",
        "chosen": "<think>\\nThis is a nuanced question. 'Wet' typically means...\\n</think>\\n\\nThis depends on how you define 'wet.' If wet means 'covered in water,' then water itself isn't wet â€” it *makes* things wet...",
...
|-------|------|------|--------|----|----|
| 0 | Preparation | Raw text corpus | â€” | â€” | Tokenizer + data pipeline |
| 1 | Pretraining | Raw text | 1 | 3e-4 | Next-token predictor |
| 2 | SFT | Instruction pairs | 1â€“3 | 2e-5 | Instruction follower |
| 3a | CoT | Reasoning traces | 1â€“2 | 1e-5 | Step-by-step thinker |
...
This is the same pipeline used by frontier models â€” just at a smaller scale. The architecture, loss functions, and training stages are identical.
...
\`\`\`
Input Tokens â†’ Embeddings â†’ [N Ã— Transformer Blocks] â†’ Output
                    â†“
            Each block contains:
...
class TokenEmbedding(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: [batch_size, seq_len] â†’ [batch_size, seq_len, d_model]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        # Project and reshape for multi-head: [batch, seq, d_model] â†’ [batch, heads, seq, d_k]
        ...
        # Concatenate heads: [batch, heads, seq, d_k] â†’ [batch, seq, d_model]
