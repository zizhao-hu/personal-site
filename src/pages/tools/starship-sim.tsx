...
type Phase = 'prelaunch' | 'ignition' | 'liftoff' | 'maxq' | 'meco' | 'separation' | 'ses' | 'coast' | 'lunar-approach' | 'landing-burn' | 'touchdown' | 'complete';
        ...
        <div className="relative w-full h-screen bg-black overflow-hidden select-none">
            ...
            <div className="absolute top-12 left-1/2 -translate-x-1/2 z-10 pointer-events-none">
                <div className={`px-4 py-1 rounded text-xs font-mono uppercase tracking-[0.2em] transition-all duration-500 ${phase === 'prelaunch' ? 'bg-white/10 text-white/50' :
                    phase === 'touchdown' || phase === 'complete' ? 'bg-green-500/20 text-green-400 border border-green-500/30' :
            ...
            <div className="absolute right-4 bottom-4 z-10 pointer-events-none">
                <div className="bg-black/70 backdrop-blur-sm border border-white/10 rounded-lg p-3 w-48" style={{ fontFamily: "'JetBrains Mono', 'Fira Code', monospace" }}>
                    ...
                    <div className="space-y-1">
                        <MissionStep label="LAUNCH" done={phase !== 'prelaunch'} active={phase === 'ignition' || phase === 'liftoff'} />
                        <MissionStep label="MAX-Q" done={['meco', 'separation', 'ses', 'coast', 'lunar-approach', 'landing-burn', 'touchdown', 'complete'].includes(phase)} active={phase === 'maxq'} />
                        <MissionStep label="MECO" done={['separation', 'ses', 'coast', 'lunar-approach', 'landing-burn', 'touchdown', 'complete'].includes(phase)} active={phase === 'meco'} />
                        <MissionStep label="SEPARATION" done={['ses', 'coast', 'lunar-approach', 'landing-burn', 'touchdown', 'complete'].includes(phase)} active={phase === 'separation'} />
                        <MissionStep label="COAST" done={['lunar-approach', 'landing-burn', 'touchdown', 'complete'].includes(phase)} active={phase === 'coast' || phase === 'ses'} />
                        <MissionStep label="LUNAR LANDING" done={['touchdown', 'complete'].includes(phase)} active={phase === 'lunar-approach' || phase === 'landing-burn'} />
                    </div>
                </div>
            </div>
